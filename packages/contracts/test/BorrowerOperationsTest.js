...
const {associateTokenWithAccount} = require("../utils/HederaOperations");
...
contract('BorrowerOperations', async accounts => {
    ...
    const testCorpus = ({withProxy = false}) => {
        beforeEach(async () => {
            ...
            contracts = await deploymentHelper.deployGasPool(contracts);
            const LQTYContracts = await deploymentHelper.deployLQTYTesterContractsHardhat(multisig)

            await deploymentHelper.connectLQTYContracts(LQTYContracts)
            await deploymentHelper.connectCoreContracts(contracts, LQTYContracts)
            await deploymentHelper.connectLQTYContractsToCore(LQTYContracts, contracts, bountyAddress, lpRewardsAddress, multisig)
            ...
            for (const account of accounts) {
                const accountToAssociate = HEDERA_LOCAL_TESTNET_ACCOUNTS.find(acc => acc.publicAddress.toLowerCase() === account.toLowerCase());
                await associateTokenWithAccount(await contracts.lusdToken.getTokenAddress(), accountToAssociate)
            }
        ...
        it("addColl(): reverts when top-up would leave trove with ICR < MCR", async () => {
            ...
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: alice}})
            await openTrove({ICR: toBN(dec(10, 8)), extraParams: {from: bob}})

            // Price drops
            await priceFeed.setPrice(dec(100, 8))
            const price = await priceFeed.getPrice()

            assert.isFalse(await troveManager.checkRecoveryMode(price))
            assert.isTrue((await troveManager.getCurrentICR(alice, price)).lt(toBN(dec(110, 6))))

            const collTopUp = 10 ** 18  // 1 wei top up
        ...
        it("addColl(): Increases the activePool ETH and raw ether balance by correct amount", async () => {
            const {collateral: aliceColl} = await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: alice}})

            const activePool_ETH_Before = toBN(await activePool.getETH() * 10 ** 10);
            ...
            const activePool_ETH_After= toBN(await activePool.getETH() * 10 ** 10);
        ...
        it("addColl(), active Trove: adds the correct collateral amount to the Trove", async () => {
            ...
            assert.isTrue(coll_After.eq(coll_before.add(toBN(dec(1, 8)))))
        ...
        it("addColl(), active Trove: updates the stake and updates the total stakes", async () => {
            ...
            assert.isTrue(alice_Stake_After.eq(alice_Stake_Before.add(toBN(dec(2, 8)))))
            assert.isTrue(totalStakes_After.eq(totalStakes_Before.add(toBN(dec(2, 8)))))
        })
        ...
        it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_LUSDDebt snapshots", async () => {
            ...
            const {
                ...
                extraLUSDAmount: toBN(dec(15000, 8)),
                ICR: toBN(dec(2, 8)),
            ...
            } = await openTrove({extraLUSDAmount: toBN(dec(10000, 8)), ICR: toBN(dec(2, 8)), extraParams: {from: bob}})
            await openTrove({extraLUSDAmount: toBN(dec(5000, 8)), ICR: toBN(dec(2, 8)), extraParams: {from: carol}})

            // --- TEST ---

            // price drops to 1ETH:100LUSD, reducing Carol's ICR below MCR
            await priceFeed.setPrice('10000000000');
            ...
            const aliceNewColl = toBN(await getTroveEntireColl(alice) * 10 ** 10);
            const aliceNewDebt = await getTroveEntireDebt(alice);
            const bobNewColl = toBN(await getTroveEntireColl(bob) * 10 ** 10);
       ...
       to the latest values of L_ETH and L_LUSDDebt */
        ...
        it("addColl(), reverts if trove is non-existent or closed", async () => {
            // A, B open troves
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: alice}})
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: bob}})
        ...
        it('addColl(): can add collateral in Recovery Mode', async () => {
            ...
            await priceFeed.setPrice('10500000000')
            ...
            assert.isTrue(aliceCollAfter.eq(aliceCollBefore.add(toBN(dec(1, 8)))))
        })
        ...
        it("withdrawColl(): reverts when calling address does not have active trove", async () => {
            await openTrove({extraLUSDAmount: toBN(dec(10000, 8)), ICR: toBN(dec(2, 8)), extraParams: {from: alice}})
            await openTrove({extraLUSDAmount: toBN(dec(10000, 8)), ICR: toBN(dec(2, 8)), extraParams: {from: bob}})

            // Bob successfully withdraws some coll
            const txBob = await borrowerOperations.withdrawColl(dec(100, 5), bob, bob, {from: bob})
            assert.isTrue(txBob.receipt.status)

            // Carol with no active trove attempts to withdraw
            try {
                const txCarol = await borrowerOperations.withdrawColl(dec(1, 8), carol, carol, {from: carol})
                assert.isFalse(txCarol.receipt.status)
            } catch (err) {
        ...
        it("withdrawColl(): reverts when requested ETH withdrawal is > the trove's collateral", async () => {
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: alice}})
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: bob}})
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: carol}})
            ...
            try {
                const txBob = await borrowerOperations.withdrawColl(bobColl.add(toBN(10000)), bob, bob, {from: bob})
                assert.isFalse(txBob.receipt.status)
            } catch (err) {
                assert.include(err.message, "revert")
            }
        ...
        it("withdrawColl(): reverts when withdrawal would bring the user's ICR < MCR", async () => {
            await openTrove({ICR: toBN(dec(10, 8)), extraParams: {from: whale}})

            await openTrove({ICR: toBN(dec(11, 7)), extraParams: {from: bob}}) // 110% ICR
        ...
        it.only("withdrawColl(): reverts if system is in Recovery Mode", async () => {
            ...
            await priceFeed.setPrice('15000000000');
        ...
        it("withdrawColl(): leaves the Trove active when the user withdraws less than all the collateral", async () => {
            ...
            await borrowerOperations.withdrawColl(dec(100, 5), alice, alice, {from: alice})
        ...
        it("withdrawColl(): reduces the Trove's collateral by the correct amount", async () => {
            ...
            await borrowerOperations.withdrawColl(dec(1, 8), alice, alice, {from: alice})

            // Check 1 ether remaining
            const alice_Trove_After = await troveManager.Troves(alice)
            const aliceCollAfter = await getTroveEntireColl(alice)

            assert.isTrue(aliceCollAfter.eq(aliceCollBefore.sub(toBN(dec(1, 8)))))
        })
        ...
        it("withdrawColl(): updates the stake and updates the total stakes", async () => {
            ...
            await openTrove({ICR: toBN(dec(2, 8)), extraParams: {from: alice, value: toBN(dec(5, 'ether'))}})
            ...
            assert.isTrue(alice_Stake_After.eq(alice_Stake_Before.sub(toBN(dec(1, 8)))))
            assert.isTrue(totalStakes_After.eq(totalStakes_Before.sub(toBN(dec(1, 8)))))
        })
       ...
       to the latest values of L_ETH and L_LUSDDebt */
        ...
        it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
            ...
            await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, LUSDRepayment, false, alice, alice, {
                    from: alice,
                    value: collTopUp
                }),
                "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
        ...
        it("adjustTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
            ...
            await assertRevert(borrowerOperations.adjustTrove(0, 0, dec(1, 18), true, A, A, {
                from: A,
                value: dec(2, 16)
            }), "Max fee percentage must be between 0.5% and 100%")
            await assertRevert(borrowerOperations.adjustTrove(1, 0, dec(1, 18), true, A, A, {
                from: A,
                value: dec(2, 16)
            }), "Max fee percentage must be between 0.5% and 100%")
            await assertRevert(borrowerOperations.adjustTrove('4999999999999999', 0, dec(1, 18), true, A, A, {
                from: A,
                value: dec(2, 16)
            }), "Max fee percentage must be between 0.5% and 100%")
        })
        ...
        it("adjustTrove(): debt increase that would leave ICR < 150% reverts in Recovery Mode", async () => {
            ...
            await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, debtIncrease, true, alice, alice, {
                    from: alice,
                    value: collIncrease
                }),
        ...
        it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
            ...
            await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, aliceDebtIncrease, true, alice, alice, {
                    from: alice,
            ...
            await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, bobDebtIncrease, true, bob, bob, {
                    from: bob,
        ...
        it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
            ...
            await borrowerOperations.adjustTrove(th._100pct, 0, await getNetBorrowingAmount(dec(50, 18)), true, alice, alice, {
                from: alice,
                value: dec(1, 'ether')
            })
        ...
        it("adjustTrove(): Changes the LUSD debt in ActivePool by requested increase", async () => {
            ...
            // Alice adjusts trove - coll increase and debt increase
            await borrowerOperations.adjustTrove(th._100pct, 0, await getNetBorrowingAmount(dec(100, 18)), true, alice, alice, {
                from: alice,
                value: dec(1, 'ether')
            })
        ...
        it("adjustTrove(): Reverts if requested coll withdrawal and ether is sent", async () => {
            ...
            await assertRevert(borrowerOperations.adjustTrove(th._100pct, dec(1, 'ether'), dec(100, 18), true, alice, alice, {
                from: alice,
                value: dec(3, 'ether')
        ...
        it("openTrove(): Opens a trove with net debt >= minimum net debt", async () => {
            ...
            const txA = await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(1))), A, A, {
                from: A,
                value: dec(100, 30)
            })
            ...
            const txC = await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(dec(47789898, 22)))), A, A, {
                from: C,
                value: dec(100, 30)
            })
        ...
        it("openTrove(): reverts if net debt < minimum net debt", async () => {
            ...
            const txBPromise = borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.sub(toBN(1))), B, B, {
                from: B,
                value: dec(100, 30)
            })
        ...
        it("openTrove(): reverts if max fee > 100%", async () => {
            await assertRevert(borrowerOperations.openTrove(dec(2, 18), dec(10000, 18), A, A, {
                from: A,
                value: dec(1000, 'ether')
            }), "Max fee percentage must be between 0.5% and 100%")
            await assertRevert(borrowerOperations.openTrove('1000000000000000001', dec(20000, 18), B, B, {
                from: B,
                value: dec(1000, 'ether')
            }), "Max fee percentage must be between 0.5% and 100%")
        })
        ...
        it("openTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
            await assertRevert(borrowerOperations.openTrove(0, dec(195000, 18), A, A, {
                from: A,
                value: dec(1200, 'ether')
            }), "Max fee percentage must be between 0.5% and 100%")
            await assertRevert(borrowerOperations.openTrove(1, dec(195000, 18), A, A, {
                from: A,
                value: dec(1000, 'ether')
            }), "Max fee percentage must be between 0.5% and 100%")
            await assertRevert(borrowerOperations.openTrove('4999999999999999', dec(195000, 18), B, B, {
                from: B,
                value: dec(1200, 'ether')
            }), "Max fee percentage must be between 0.5% and 100%")
        })
        ...
        it("openTrove(): reverts if fee exceeds max fee percentage", async () => {
            ...
            const lessThan5pct = '49999999999999999'
            await assertRevert(borrowerOperations.openTrove(lessThan5pct, dec(30000, 18), A, A, {
                from: D,
                value: dec(1000, 'ether')
            ...
            assert.equal(borrowingRate, dec(5, 16))
            // Attempt with maxFee 1%
            await assertRevert(borrowerOperations.openTrove(dec(1, 16), dec(30000, 18), A, A, {
                from: D,
                value: dec(1000, 'ether')
            }), "Fee exceeded provided maximum")
            ...
            assert.equal(borrowingRate, dec(5, 16))
            // Attempt with maxFee 3.754%
            await assertRevert(borrowerOperations.openTrove(dec(3754, 13), dec(30000, 18), A, A, {
                from: D,
                value: dec(1000, 'ether')
            }), "Fee exceeded provided maximum")
            ...
            await assertRevert(borrowerOperations.openTrove(dec(5, 15), dec(30000, 18), A, A, {
                from: D,
                value: dec(1000, 'ether')
            }), "Fee exceeded provided maximum")
        })
        ...
        it("openTrove(): Reverts opening a trove with min debt when system is in Recovery Mode", async () => {
            ...
            await assertRevert(borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT), carol, carol, {
                from: carol,
                value: dec(1, 'ether')
            }))
        })
