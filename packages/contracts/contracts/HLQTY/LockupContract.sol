...
* Within the first year from deployment, the deployer of the LQTYToken (Liquity AG's address) may transfer HLQTY only to valid
* LockupContracts, and no other addresses (this is enforced in LQTYToken.sol's transfer() function).
* 
* The above two restrictions ensure that until one year after system deployment, HLQTY tokens originating from Liquity AG cannot
...
contract LockupContract is BaseHST {
    ...
    function withdrawHLQTY() external {
        _requireCallerIsBeneficiary();
        _requireLockupDurationHasPassed();

        IHLQTYToken hlqtyTokenCached = hlqtyToken;
        uint LQTYBalance = hlqtyTokenCached.balanceOf(address(this));

        _transfer(hlqtyToken.getTokenAddress(), address(this), beneficiary, LQTYBalance);
