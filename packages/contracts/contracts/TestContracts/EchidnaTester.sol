...
contract EchidnaTester {
    ...
    uint private MCR;
    uint private CCR;
    uint private DCHF_GAS_COMPENSATION;
    ...
    constructor() public payable {
        ...
        dchfToken = new DCHFToken(
            address(troveManager),
            address(stabilityPool),
            address(borrowerOperations)
        );

        gasPool = new GasPool(address(dchfToken), address(troveManager), address(borrowerOperations));
        ...
        troveManager.setAddresses(address(borrowerOperations), 
            ...
            address(priceFeedTestnet), address(dchfToken),
        ...
        stabilityPool.setAddresses(address(borrowerOperations), 
            address(troveManager), address(activePool), address(dchfToken),
        ...
        for (uint i = 0; i < NUMBER_OF_ACTORS; i++) {
            echidnaProxies[i] = new EchidnaProxy(troveManager, borrowerOperations, stabilityPool, dchfToken);
        ...
        DCHF_GAS_COMPENSATION = borrowerOperations.DCHF_GAS_COMPENSATION();
        require(MCR > 0);
        require(CCR > 0);
    ...
    function redeemCollateralExt(
        ...
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].redeemCollateralPrx(_DCHFAmount, _firstRedemptionHint, _upperPartialRedemptionHint, _lowerPartialRedemptionHint, _partialRedemptionHintNICR, 0, 0);
    }
    ...
    function getAdjustedETH(uint actorBalance, uint _ETH, uint ratio) internal view returns (uint) {
        ...
        require(price > 0);
        uint minETH = ratio.mul(DCHF_GAS_COMPENSATION).div(price);
        require(actorBalance > minETH);
        uint ETH = minETH + _ETH % (actorBalance - minETH);
        return ETH;
    }

    function getAdjustedDCHF(uint ETH, uint _DCHFAmount, uint ratio) internal view returns (uint) {
        uint price = priceFeedTestnet.getPrice();
        uint DCHFAmount = _DCHFAmount;
        uint compositeDebt = DCHFAmount.add(DCHF_GAS_COMPENSATION);
        uint ICR = LiquityMath._computeCR(ETH, compositeDebt, price);
        if (ICR < ratio) {
            compositeDebt = ETH.mul(price).div(ratio);
            DCHFAmount = compositeDebt.sub(DCHF_GAS_COMPENSATION);
        }
        return DCHFAmount;
    }

    function openTroveExt(uint _i, uint _ETH, uint _DCHFAmount) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        ...
        uint DCHFAmount = getAdjustedDCHF(ETH, _DCHFAmount, CCR);

        //console.log('ETH', ETH);
        //console.log('DCHFAmount', DCHFAmount);

        echidnaProxy.openTrovePrx(ETH, DCHFAmount, address(0), address(0), 0);
    ...
    function openTroveRawExt(uint _i, uint _ETH, uint _DCHFAmount, address _upperHint, address _lowerHint, uint _maxFee) public payable {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].openTrovePrx(_ETH, _DCHFAmount, _upperHint, _lowerHint, _maxFee);
    }
    ...
    function withdrawDCHFExt(uint _i, uint _amount, address _upperHint, address _lowerHint, uint _maxFee) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].withdrawDCHFPrx(_amount, _upperHint, _lowerHint, _maxFee);
    }

    function repayDCHFExt(uint _i, uint _amount, address _upperHint, address _lowerHint) external {
        uint actor = _i % NUMBER_OF_ACTORS;
        echidnaProxies[actor].repayDCHFPrx(_amount, _upperHint, _lowerHint);
    }
    ...
    function adjustTroveExt(uint _i, uint _ETH, uint _collWithdrawal, uint _debtChange, bool _isDebtIncrease) external payable {
        ...
        if (_isDebtIncrease) {
            ...
            debtChange = getAdjustedDCHF(ETH, uint(_debtChange), MCR);
        }
    ...
    function echidna_trove_properties() public view returns(bool) {
        ...
        while (currentTrove != address(0)) {
            ...
            // Minimum debt (gas compensation)
            if (troveManager.getTroveDebt(currentTrove) < DCHF_GAS_COMPENSATION) {
                return false;
            }
            // Uncomment to check that the condition is meaningful
            //else return false;
    ...
    function echidna_ETH_balances() public view returns(bool) {
        ...
        if (address(dchfToken).balance > 0) {
            return false;
        }
    ...
    // Total DCHF matches
    function echidna_DCHF_global_balances() public view returns(bool) {
        uint totalSupply = dchfToken.totalSupply();
        uint gasPoolBalance = dchfToken.balanceOf(address(gasPool));

        uint activePoolBalance = activePool.getDCHFDebt();
        uint defaultPoolBalance = defaultPool.getDCHFDebt();
        if (totalSupply != activePoolBalance + defaultPoolBalance) {
            return false;
        }

        uint stabilityPoolBalance = stabilityPool.getTotalDCHFDeposits();
        address currentTrove = sortedTroves.getFirst();
        uint trovesBalance;
        while (currentTrove != address(0)) {
            trovesBalance += dchfToken.balanceOf(address(currentTrove));
            currentTrove = sortedTroves.getNext(currentTrove);
        }
