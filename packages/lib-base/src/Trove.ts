...
export class Trove {
  ...
  /** @internal */
  constructor(constants: Constants, collateral = Decimal.ZERO, debt = Decimal.ZERO) {
  ...
  apply(
    ...
    switch (change.type) {
      ...
      case "closure":
        ...
        return new Trove(this.constants);
  ...
  static recreate(that: Trove, borrowingRate?: Decimalish): TroveCreationParams<Decimal> {
    const change = new Trove(that.constants).whatChanged(that, borrowingRate);
