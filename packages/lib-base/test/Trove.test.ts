...
import { defaults as constants } from "../src/constants";

import { Decimal, Difference } from "../src/Decimal";
import { Trove } from "../src/Trove";

const liquidationReserve = Number(constants.HCHF_LIQUIDATION_RESERVE);
const maximumBorrowingRate = Number(constants.MAXIMUM_BORROWING_RATE);

const maxDebt = 10 * Number(constants.HCHF_MINIMUM_DEBT);

const trove = ({ collateral = 0, debt = 0 }) =>
  new Trove(constants, Decimal.from(collateral), Decimal.from(debt));
...
const roughlyEqual = (a: Trove, b: Trove) =>
  a.collateral.eq(b.collateral) && !!Difference.between(a.debt, b.debt).absoluteValue?.lt(1e-4);

describe("Trove", () => {
  it("applying undefined diff should yield the same Trove", () => {
    const trove = new Trove(constants, Decimal.from(1), Decimal.from(111));
  ...
  it("applying diff of empty from `b` to `a` should yield empty", () => {
    fc.assert(
      fc.property(validNonEmptyTrove(), validNonEmptyTrove(), (a, b) =>
        a.apply(b.whatChanged(new Trove(constants))).equals(new Trove(constants))
      )
    );
  });
        ...
        fc.property(validNonEmptyTrove(), validNonEmptyTrove(), (a, b) => {
          ...
          return (
            ...
            (change.type === "adjustment" &&
              ...
              !change.params.borrowHCHF?.isZero &&
              !change.params.repayHCHF?.isZero)
          );
        })
      );
    });
      ...
      fc.assert(
        fc.property(fc.float({ max: maximumBorrowingRate }), borrowingRate => {
          const withMinimumDebt = Trove.recreate(
            new Trove(constants, Decimal.ONE, constants.HCHF_MINIMUM_DEBT),
            borrowingRate
          );

          const ret = Trove.create(constants, withMinimumDebt, borrowingRate).debt.gte(
            constants.HCHF_MINIMUM_DEBT
          );

          if (!ret) {
            console.log(`${Trove.create(constants, withMinimumDebt, borrowingRate).debt}`);
          }
