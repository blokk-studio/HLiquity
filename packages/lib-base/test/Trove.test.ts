...
const liquidationReserve = Number(HCHF_LIQUIDATION_RESERVE);
const maximumBorrowingRate = Number(MAXIMUM_BORROWING_RATE);

const maxDebt = 10 * Number(HCHF_MINIMUM_DEBT);
            ...
            fc.assert(
                fc.property(fc.float({ max: maximumBorrowingRate }), borrowingRate => {
                    const withMinimumDebt = Trove.recreate(
                        new Trove(Decimal.ONE, HCHF_MINIMUM_DEBT),
                        borrowingRate
                    );

                    const ret = Trove.create(withMinimumDebt, borrowingRate).debt.gte(HCHF_MINIMUM_DEBT);
