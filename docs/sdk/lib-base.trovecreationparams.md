...
```typescript
export declare type TroveCreationParams<T = unknown> = _CollateralDeposit<T> & _NoCollateralWithdrawal & _HCHFBorrowing<T> & _NoHCHFRepayment;
```
...
<tr> <td> borrowHCHF </td> <td> T </td> <td> The amount of HCHF that's borrowed. </td> </tr>
