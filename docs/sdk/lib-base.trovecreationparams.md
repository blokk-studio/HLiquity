...
```typescript
export declare type TroveCreationParams<T = unknown> = _CollateralDeposit<T> & _NoCollateralWithdrawal & _DCHFBorrowing<T> & _NoDCHFRepayment;
```
...
<tr> <td> borrowDCHF </td> <td> T </td> <td> The amount of DCHF that's borrowed. </td> </tr>
